...
public class SpawnCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> context, EntitySelector selector) throws CommandSyntaxException {

        List<? extends Entity> entities;
        if(selector == null) {
            entities = Collections.singletonList(context.getSource().getEntity());
            CommandUtil.sendCommandSuccess(context, MidnightEssentialsAPI.getInstance().getLangProvider(), false, "command.spawn.success");

        } else {
            entities = selector.findEntities(context.getSource());
            CommandUtil.sendCommandSuccess(context, MidnightEssentialsAPI.getInstance().getLangProvider(), false, "command.spawn.success.other", CustomPlaceholderInline.create("count", entities.size()+""));
        }
    ...
    private static int executeSet(CommandContext<CommandSourceStack> context, Coordinates coords, Coordinates rotation, ResourceLocation level) {
        ...
        CommandUtil.sendCommandSuccess(context, MidnightEssentialsAPI.getInstance().getLangProvider(), false, "command.setspawn.success");
